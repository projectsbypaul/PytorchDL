...
class UNet3D_16ELGN(nn.Module):
    def __init__(self, in_channels=1, out_channels=7, apply_softmax=False):
        ...
        self.apply_softmax = apply_softmax
        # Encoder (16³ -> 8³)
        self.encoder1 = ConvBlock3DGN(in_channels, 64)
        self.pool1 = nn.MaxPool3d(2)  # -> 64 × 8³

        # Bottleneck
        self.bottleneck = ConvBlock3DGN(64, 128)  # -> 128 × 8³

        # Decoder
        self.upconv1 = nn.ConvTranspose3d(128, 64, kernel_size=2, stride=2)  # -> 64 × 16³
        self.decoder1 = ConvBlock3DGN(128, 64)  # skip connection from encoder1 (64+64)
...
class UNet_HilbigGN(nn.Module):
    ...
    def __init__(self, in_channels=1, out_channels=7, apply_softmax=False):
        ...
        # ---- Encoder ----
        self.enc1a = ConvGNReLU3D(in_channels, 64)
        self.enc1b = ConvGNReLU3D(64, 64)

        self.pool  = nn.MaxPool3d(2) #64x32³ -> 64x16³

        self.enc2a = ConvGNReLU3D(64, 128)
        self.enc2b = ConvGNReLU3D(128, 128)

        # ---- Bottleneck ----
        self.bott1 = ConvGNReLU3D(128, 256)
        self.bott2 = ConvGNReLU3D(256, 256)

        # ---- Decoder ----
        self.up2   = nn.ConvTranspose3d(256, 128, kernel_size=2, stride=2)
        self.dec2a = ConvGNReLU3D(128 + 128, 128)
        self.dec2b = ConvGNReLU3D(128, 128)

        self.up1   = nn.ConvTranspose3d(128, 64, kernel_size=2, stride=2)
        self.dec1a = ConvGNReLU3D(64 + 64, 64)
        self.dec1b = ConvGNReLU3D(64, 64)
