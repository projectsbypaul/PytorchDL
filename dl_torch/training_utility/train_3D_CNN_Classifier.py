...
from dl_torch.models.CNN3D_Classifier import CNN3D_Classifier, CNN3D_Classifier_MLPlus
...
def train_model(model,
    ...
    for epoch in range(num_epochs):
        ...
        for data, target in train_loader:
            ...
            target = torch.argmax(target, 1)  # get index of max value entry
            _, predicted = torch.max(output, 1) # get index of 1-hot encoded entry
            total += target.size(0) # a batch to total amount of passed sample in an epoch
            correct += (predicted == target).sum().item() # compare prediction and labels and count correct predictions

        #output epoch results
        train_acc = 100.0 * correct / total
        print(
            f"Epoch {epoch + 1}/{num_epochs}, Loss: {train_loss / len(train_loader):.8f}, Train Accuracy: {train_acc:.2f}%")
        ...
        with torch.no_grad():
            for data, target in val_loader:
                ...
                target = torch.argmax(target, dim=1) # get index of 1-hot encoded entry
                val_total += target.size(0)
                val_correct += (predicted == target).sum().item()

        val_acc = 100.0 * val_correct / val_total
        print(f"Val_Loss: {val_loss / len(val_loader):.8f}, Validation Accuracy: {val_acc:.2f}% | Learning Rate: {scheduler.get_last_lr()[0]:.6f}")
...
def training_routine():
    ...
    # Save data
    model_save_name = "CNN_Classifier_n10_MLPlus"
    backup_epoch = 100
    dataset_loc = r"../../data/datasets/ModelNet10/ModelNet10_SDF_32_train.torch"
    model_weights_loc = "../../data/model_weights/CNN_Classifier_n10_MLPlus/{}.pth"
    ...
    model = CNN3D_Classifier_MLPlus(num_classes)
...
def validation_routine():
    ...
    # Save data
    model_save_name = "CNN_Classifier_n10_MLPlus"
    dataset_loc = r"../../data/datasets/ModelNet10/ModelNet10_SDF_32_test.torch"
    ...
    # Setup model
    loaded_model = CNN3D_Classifier_MLPlus(num_classes)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
